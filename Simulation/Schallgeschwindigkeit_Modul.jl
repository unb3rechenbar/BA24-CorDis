...
VelocityOfSound(k, D_k) = mean(sqrt.([E^(0.5)/q for (E,q) in zip(D_k,k) if E > 0 && q > 0]))
...
if occursin("-S", ARGS[1])
    if occursin("-exp", ARGS[1])
        CSV.write("VelocityOfSound-S-exp.csv", DataFrame(v = [v], density = [density], resolution = [resolution]), append = true)
    else
        CSV.write("VelocityOfSound-S.csv", DataFrame(v = [v], density = [density], resolution = [resolution]), append = true)
    end
else
    if occursin("-exp", ARGS[1])
        CSV.write("VelocityOfSound-nS-exp.csv", DataFrame(v = [v], density = [density], resolution = [resolution]), append = true)
    else
        CSV.write("VelocityOfSound-nS.csv", DataFrame(v = [v], density = [density], resolution = [resolution]), append = true)
    end
end